...
d = collections.OrderedDict() # 字典的子类，保存了他们被添加的顺序
...
for method_name in d.keys():
    ...
    for severity in range(1,9):
        ...
        for img, label in zip(test_data.data, test_data.targets):
            # print(img.shape)
            # print(corruption(img).shape)
            # print(img.dtype)
            # convert_img(corruption(img)).save('./pics/4_.jpg')
            labels.append(label)
            try:
                cifar_c.append(np.uint8(corruption(convert_img(img))))
            except:
                cifar_c.append(np.uint8(corruption(img)))
        np.save(des_path + d[method_name].__name__ + str(severity) + '.npy',
            np.array(cifar_c).astype(np.uint8))
    # np.save(des_path + d[method_name].__name__ + str(severity) + '_02' + '.npy',
    #     np.array(cifar_c).astype(np.uint8))

    np.save(label_path, np.array(labels).astype(np.uint8))